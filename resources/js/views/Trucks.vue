    ...
    <div class="trucks-management">
        ...
        <div class="table-page-header">
            ...
            <div class="table-page-actions">
                <a href="/api/reports/trucks" target="_blank" class="btn btn-outline-success">
        ...
        <div v-else class="card">
            ...
            <div class="table-responsive">
                <table class="table table-hover">
                    ...
                    <tbody>
                        <tr v-for="(truck, index) in trucks" :key="truck.id">
                            <td><strong>{{ getSerialNumber(index) }}</strong></td>
                            <td>{{ truck.plate_number }}</td>
                            <td>{{ truck.brand }} {{ truck.model }}</td>
                            <td>{{ truck.year }}</td>
                            <td>
                                <span :class="getStatusBadgeClass(truck.status)">
                                    {{ truck.status }}
                                </span>
                            </td>
                            <td>{{ truck.driver ? truck.driver.name : 'Unassigned' }}</td>
                            <td>
                                <button class="btn btn-sm btn-info me-2" @click="editTruck(truck)">
                        ...
                        <tr v-if="trucks.length === 0">
                            <td colspan="7" class="text-center">No trucks found.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
                ...
                <nav>
                    <ul class="pagination mb-0">
                        <li class="page-item" :class="{ disabled: pagination.current_page === 1 }">
                            <button class="page-link" @click="fetchTrucks(1)"><i class="bi bi-chevron-double-left"></i></button>
                        </li>
                        <li class="page-item" :class="{ disabled: pagination.current_page === 1 }">
                            <button class="page-link" @click="fetchTrucks(pagination.current_page - 1)">Previous</button>
                        </li>
                        <li v-for="page in getPageNumbers()" :key="page" class="page-item" :class="{ active: page === pagination.current_page }">
                            <button class="page-link" @click="fetchTrucks(page)">{{ page }}</button>
                        </li>
                        <li class="page-item" :class="{ disabled: pagination.current_page === pagination.last_page }">
                            <button class="page-link" @click="fetchTrucks(pagination.current_page + 1)">Next</button>
                        </li>
                        <li class="page-item" :class="{ disabled: pagination.current_page === pagination.last_page }">
                            <button class="page-link" @click="fetchTrucks(pagination.last_page)"><i class="bi bi-chevron-double-right"></i></button>
                        </li>
                    </ul>
                </nav>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ isEditing ? 'Edit Truck' : 'Add Truck' }}</h5>
                ...
                <div class="modal-body">
                    <form @submit.prevent="saveTruck">
                        <div class="mb-3">
                            ...
                            <input v-model="form.plate_number" type="text" class="form-control" required>
                        </div>
                        <div class="mb-3">
...
const saveTruck = async () => {
    try {
        if (isEditing.value) {
            await axios.put(`/api/trucks/${form.id}`, form);
        } else {
            await axios.post('/api/trucks', form);
        }
        closeModal();
        fetchTrucks(pagination.value.current_page);
    } catch (error) {
        console.error('Error saving truck:', error);
        alert('Failed to save truck. Please check inputs.');
    }
};
